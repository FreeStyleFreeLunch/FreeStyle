...
class StableDiffusionXLImg2ImgPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin):
    ...
    def __call__(
        ...
        refimg=self.load_img(refimgpath)
        refimg=self.image_processor.preprocess(refimg)
        # 5. Prepare timesteps
        ...
        print("latent shape: ",latent_timestep)
        ...
        """reflatents = self.prepare_latents(
            refimg,
            latent_timestep,
            batch_size,
            num_images_per_prompt,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                skipstack=None
                reflatents = self.prepare_latents(image,
                                                latent_timestep,
                                                batch_size,
                                                num_images_per_prompt,
                                                prompt_embeds.dtype,
                ...
                noise_pred,skipstack = self.unet(
                    latent_model_input,
                    t,
